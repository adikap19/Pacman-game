...
//This function manages the running game until the user losing/wining it
{
	...
	int ghostStepsCounter = 1, fruitStepsCounter = 1, flag = 1;
	char pacDir = STAY_KEY, levelChoice = setGhostsLevel();

	initVectors(behindGhosts, ghostsDir, behindFruits);
	...
	while (!endTheGame)
	{
		...
		setPoints(points + fruitsPoints);
		flag *= -1;

		ghostAndPacMeeting(behindGhosts);
		fruitsAndOtherCreaturesMeeting(fruitStepsCounter, behindFruits);

		if(theGame->getGameBoard().getMaxBreadCrumbs() == points || lives == 0)
		{
...
void GameWoManager::initVectors(vector<char>& behindGhosts, vector<char>& ghostsDir, vector<char>& behindFruit)
//This function inits the help vector that we made in manageGame function
{
	for (int i = 0; i < theGame->getGhostsAmount(); i++)
	{
	...
	{
		Position behindFruitI = theGame->getFruitsVec()[i].getPos();
		if(theGame->getGameBoard().getCellFromBoard(behindFruitI) == BREADCRUMB_SIGN)
			behindFruit.push_back(BREADCRUMB_SIGN);
		else if (theGame->getGameBoard().getCellFromBoard(behindFruitI) == ' ')
			behindFruit.push_back(' ');	
		theGame->getGameBoard().setCell(theGame->getFruitsVec()[i].getSign(), behindFruitI);
	}
}

void GameWoManager::ghostAndPacMeeting(vector<char>& behindGhosts)
...
void GameWoManager::fruitsAndOtherCreaturesMeeting(int& fruitStepsCounter, vector<char>& behindFruit)
//This function handles the fruits and ghost/pacWoman meetings
{
	for (int i = theGame->getFruitsAmount() - 1; i >= 0; i--)
	{
		if (theGame->getFruitsVec()[i].getAppear() == true)
		{
			...
			{
				for (int j = 0; j < theGame->getGhostsAmount(); j++)
